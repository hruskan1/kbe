...
The task is implemented in file `dh_alg.py`.
...
## Task 3: Bulk cipher
Ensure you have working implementation of AES in CBC mode with PKCS7 padding. It is recommended to use  `BLOCK_SIZE = 16`
...
The task is implemented in file `bulk_cipher.py`.
...
The task is implemented in file `tls_101.py`
...
The task is implemented in `itls_101.py`
...
The task is implemented in file `check_rsa.py`. The functions are implemented in `rsa_alg.py`.
...
We can deduce the value of `M^3` in field Z_{N_1N_2N_3} and reduce the problem of finding the root of number in Z_k to finding root in Z as M < N_i for all i and therfore M^3 < N_1N_2N_3.

The task is implemented in file `rsa_broadcast_attack.py`
...
The task is implemented in file `rsa_bleichenbachers.py`.
...
The task is implemented in `dsa_attack.py`.
...
I did not implemented this bonus task. 